...
def post_list(request):
    ...
    selected_tags = [int(tag) for tag in request.GET.getlist('tag') if tag.isdigit()]
    ...
    if 'all' in selected_catalogs:
        ...
        posts = posts.filter(catalog__id__in=selected_catalogs)

    if selected_tags:
        posts = posts.filter(tag__id__in=selected_tags)

    if search_query:
        posts = posts.filter(title__icontains=search_query)  # `name` o'rniga `title`
...
def blog_detail(request, year, month, day, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            post.comments.create(  # `Comment.objects.create(...)` o‘rniga soddalashtirildi
    ...
    comments = post.comments.all()  # `Comment.objects.filter(post=post)` o‘rniga soddalashtirildi
